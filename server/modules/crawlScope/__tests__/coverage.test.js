...
// The asset checks read internal files only, so a complete crawl has some: a
// site with none (every file on a CDN) has those checks "not evaluated".
const asset = (path, contentType) => ({
  url: `${H}${path}`, scope: "Internal", status: 200, statusText: "OK", contentType, isAsset: true,
  cacheable: true, contentEncoding: "gzip", size: 2048,
});
const assets = [asset("/app.js", "application/javascript"), asset("/site.css", "text/css"), asset("/logo.png", "image/png")];
const coverageOf = (options) =>
  buildFindings({ results: [page("/"), ...assets], linkEdges: [], startUrl: `${H}/`, ...fullSite, ...options }).coverage;
